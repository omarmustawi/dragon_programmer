    ...
    <tr key={id} className="bg-white border-b relative hover:bg-slate-200">
      ...
      {header.map((cell, id) => (
        <td key={id} className="py-3 px-6">
          {
            location.pathname.includes("allCoursesControllPannel") ? ( // A)
              cell.key === "name" ? (
                row.teacher[cell.key]
              ) : (
                row.course[cell.key]
              )
            ) : cell.key === "role" ? ( // B- a)
              ...
              <img
                className="w-36 h-32"
                src={`http://127.0.0.1:8000/images/${row[cell.key]}`}
                alt=""
              />
            ) : (
              row[cell.key]
      ...
      <td className="flex justify-around items-center align-baseline text-violet-600 py-3">
        ...
        {location.pathname.includes("newRegistration") ? (
          ...
          <>
            <Link to={`${data[id].course.id}`}>
              <FiEdit size={30} />
            </Link>
            <Link to={`addVideo/${data[id].course.id}`}>
